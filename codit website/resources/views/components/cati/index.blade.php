        ...
        <div class="card mb-5">
            <div class="card-header">
                <div class="d-flex justify-content-between">
                    <div>{{ __('words.catigury') }}</div>
                    <div class="option">
                        <a href="{{ route("cati.create") }}" class="fa fa-plus py-1 px-2"> {{ __("control.add") }} </a>
                        <a href="{{ route('cati.index') }}" class="fa fa-external-link py-1 px-2"> {{ __("control.back") }} </a>
                ...
                <table class="table table-bordered table-hover table-inverse table-striped table-responsive">
                    <thead class="thead-inverse">
                        <tr>
                            ...
                            <th class="w-50 text-center">{{ __("words.catigury") }}</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><input type="checkbox" data-for="{{ $cati->id }}" class="form-check mt-2"></td>
                            <td>{{ $cati->id }}</td>
                            <td>{{ $cati->label }}</td>
                            <td class="text-center">
                                <button data-for="{{ $cati->id }}" class="cati_edit btn btn-sm btn-outline-secondary m-1">{{ __("control.edit") }}</button>
                                <button data-for="{{ $cati->id }}" class="cati_delete btn btn-sm btn-outline-danger m-1">{{ __("control.delete") }}</button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
...
<script>
    $(function() {
        ...
        $('.cati_edit').on('click', function(e) {
            event.preventDefault();
            $(forms).prop('method', 'GET');
            $("#action_form input[name='_token']").remove();
            $(forms).prop('action', '{{ route("cati.index") }}/' + $(this).attr("data-for") + "/edit");
            $(forms).submit();
        });
        $('.cati_delete').on('click', function(e) {
            event.preventDefault();
            $(forms).prop('method', 'POST');
            $('<input type="hidden" name="_method" value="delete">').insertBefore("#action_form>input");
            $(forms).prop('action', '{{ route("cati.index") }}/' + $(this).attr("data-for"));
            $(forms).submit();
        });
                ...
                headers: {
                 ...
                 //request success function with responce
                  success: function(data) {
                      //console.log(data);
                    $(forms).prop('method', 'GET');
                    $(forms).prop('action', '{{ route("cati.index") }}');
                    $(forms).submit();
                    $("#loading").remove();
                },
