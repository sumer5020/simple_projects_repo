        ...
        <div class="card mb-5">
            <div class="card-header">
                <div class="d-flex justify-content-between">
                    <div>{{ trans_choice("words.offer",3) }}</div>
                    <div class="option">
                        <a href="{{ route("offer.create") }}" class="fa fa-plus py-1 px-2"> {{ __("control.add") }} </a>
                ...
                <table class="table table-bordered table-hover table-inverse table-striped table-responsive">
                    <thead class="thead-inverse">
                        <tr>
                            ...
                            <th class="text-center">{{ __("words.cost") }}</th>
                            <th class="text-center"><i class="fa fa-money"></i></th>
                            <th class="w-25">{{ __("words.title") }}</th>
                            <th class="w-50">{{ __("words.content") }}</th>
                            <th class="text-center">{{ __("control.startDate") }}</th>
                            <th class="text-center">{{ __("control.endDate") }}</th>
                    ...
                    <tbody>
                        @foreach($offers as $offer)
                            <tr>
                                <td><input type="checkbox" data-for="{{ $offer->id }}" class="form-check mt-2"></td>
                                <td>{{ $offer->id }}</td>
                                <td>{{ $offer->cati['label'] }}</td>
                                <td>{{ $offer->cost }}$</td>
                                <td>{{ $offer->pay_count }}</td>
                                <td><a href="/offer/{{ $offer->id }}">{{app()->getLocale()=='en'? $offer->title:$offer->title_ar }}</a></td>
                                <td>{{app()->getLocale()=='en'? $offer->desc:$offer->desc_ar }}</td>
                                <td>{{ $offer->start_at }}</td>
                                <td>{{ $offer->end_at }}</td>
                                <td class="text-center">
                                    <button data-for="{{ $offer->id }}" class="card_edit btn btn-sm btn-outline-secondary m-1">{{ __("control.edit") }}</button>
                                    <button data-for="{{ $offer->id }}" class="card_delete btn btn-sm btn-outline-danger m-1">{{ __("control.delete") }}</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
...
<script>
    $(function(){
        ...
        $('.card_edit').on('click', function(e) {
            ...
            $(forms).prop('action', '{{ route("offer.index") }}/' + $(this).attr("data-for") + "/edit");
            $(forms).submit();
        });
        ...
        $('.card_delete').on('click', function(e) {
            ...
            $(forms).prop('action', '{{ route("offer.index") }}/' + $(this).attr("data-for"));
            $(forms).submit();
        });
                ...
                headers: {
                 ...
                 //request success function with responce
                  success: function(data) {
                      //console.log(data);
                    $(forms).prop('method', 'GET');
                        $(forms).prop('action', '{{ route("offer.index") }}');
                        $(forms).submit();
                    $("#loading").remove();
                },
