        ...
        <div class="card mb-5">
            <div class="card-header">
                <div class="d-flex justify-content-between">
                    <div>{{ trans_choice("words.headerCard",3) }}</div>
                    <div class="option">
                        <a href="{{ route("Cards.create") }}" class="fa fa-plus py-1 px-2"> {{ __("control.add") }} </a>
                ...
                <table class="table table-bordered table-hover table-inverse table-striped table-responsive">
                    <thead class="thead-inverse">
                        <tr>
                            ...
                            <th class="text-center">{{ __("words.simpol") }}</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><input type="checkbox" data-for="{{ $card->id }}" class="form-check mt-2"></td>
                            <td>{{ $card->id }}</td>
                            <td><i class="simpol fa {{ $card->icon }}"></i></td>
                            <td><a href="/Cards/{{ $card->id }}">{{app()->getLocale()=='en'? $card->title:$card->title_ar }}</a></td>
                            <td>{{app()->getLocale()=='en'? $card->desc:$card->desc_ar }}</td>
                            <td class="text-center">
                                <button data-for="{{ $card->id }}" class="card_edit btn btn-sm btn-outline-secondary m-1">{{ __("control.edit") }}</button>
                                <button data-for="{{ $card->id }}" class="card_delete btn btn-sm btn-outline-danger m-1">{{ __("control.delete") }}</button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
...
<script>
    $(function() {
        ...
        $('.card_edit').on('click', function(e) {
            ...
            $(forms).prop('action', '{{ route("Cards.index") }}/' + $(this).attr("data-for") + "/edit");
            $(forms).submit();
        });
        $('.card_delete').on('click', function(e) {
            ...
            $(forms).prop('action', '{{ route("Cards.index") }}/' + $(this).attr("data-for"));
            $(forms).submit();
        });
                ...
                headers: {
                 ...
                 //request success function with responce
                  success: function(data) {
                      //console.log(data);
                    $(forms).prop('method', 'GET');
                        $(forms).prop('action', '{{ route("Cards.index") }}');
                        $(forms).submit();
                    $("#loading").remove();
                },
